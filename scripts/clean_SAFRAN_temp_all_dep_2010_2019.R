...
temp_2010_2019<- temp_2010_2019  %>%
  ...
  temp_2010_2019_1<- temp_2010_2019 %>%
...
# merge both lambert files
 ...
 temp_2010_2019_1 <- temp_2010_2019_1 %>%
   left_join(lambert_both)%>%
   filter(!is.na(num_maille))# remove maille outside France (CH DE BE)

 #rm(lambert_conv,lambert_conv_ext,lambert_both, temp_20_25, temp_2010_2019)


# ## Only Britanny to begin ####
# temp_2010_2019_Bret <- temp_2010_2019_1 %>%
#   filter(num_dep==35 | num_dep==22| num_dep==29| num_dep==56)
#  temp_20_Bret <- temp_2010_2019_Bret %>%
#    filter(year==2020)
#  temp_20_Bret_oneday <- temp_2010_2019_Bret %>%
#    filter(DATE==20220101)
#  temp_20_Bret_3days <- temp_2010_2019_Bret %>%
#    filter(DATE==20200101 | DATE==20200102 | DATE==20200103)
# ### Hottest day of summer 2022 ####
# summer2022_hottest_day <- temp_2010_2019_Bret %>%
...
# # only Alsace
#  temp_2010_2019_Als <- temp_2010_2019_1 %>%
#    filter(num_dep==67 | num_dep==68)
#  temp_20_Als_oneday <- temp_2010_2019_Als %>%
#    filter(DATE==20200101)
#  temp_20_Als_3days <- temp_2010_2019_Als %>%
#    filter(DATE==20200101 | DATE==20200102 | DATE==20200103)

 
 
# ## Whole France but only one day of 2020 ####
# temp_20_France_oneday <- temp_2010_2019_1 %>%
#   filter(DATE==20220101)
# ## Whole France but only 3days of 2020 ####
#  temp_France_2020_3d <- temp_2010_2019_1 %>%
#    filter(DATE==20220101|DATE==20220102 | DATE==20220103)
# ## Whole France but only 2020 ####
#  temp_France_2020 <- temp_2010_2019_1 %>%
#    filter(year==2020)
 rm(lambert_conv,lambert_conv_ext,lambert_both, temp_2010_2019)
 temp_2010_2019_1_summer <- temp_2010_2019_1 %>%
